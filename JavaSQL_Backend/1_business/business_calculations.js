...
async function getAllAccounts(userId) {
  return await transactionDB.getAllAccounts(userId);
}
...
async function createAccount(name, userId) {
  ...
  return await transactionDB.createAccount(name.trim(), userId);
}

// Rename an account
async function renameAccount(id, newName, userId) {
  if (!newName || newName.trim() === "") {
    throw new Error("New name is required.");
  }
  return await transactionDB.renameAccount(id, newName.trim(), userId);
}

// Delete an account
async function deleteAccount(id, userId) {
  return await transactionDB.deleteAccount(id, userId);
}

// ENTRIES

// Get all ledger entries for an account
async function getEntries(accountId, userId) {
  if (!accountId) throw new Error("accountId is required.");
  return await transactionDB.getEntries(accountId, userId);
}

// Add a ledger entry
async function addEntry(data, userId) {
